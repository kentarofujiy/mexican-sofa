...
class Comfy::Admin::Cms::SnippetsController < Comfy::Admin::Cms::BaseController
  ...
  def create
    @snippet.save!
      nome = @current_user.name +  ' CRIOU  ' + @snippet.identifier
    detail = @snippet.to_yaml
    action = Useraction.create(nome: nome, description: detail, severity: 3)
    action.save
  ...
  def update
    @snippet.update!(snippet_params)
        nome = @current_user.name + ' EDITOU FRAG ' + @snippet.identifier
    detail = @snippet.to_yaml
    action = Useraction.create(nome: nome, description: detail, severity: 2)
    action.save
  ...
  def destroy
    @snippet.destroy
      nome = @current_user.name + ' DELETOU FRAG' + @snippet.identifier
    action = Useraction.create(nome: nome, description: DateTime.current.strftime('%d/%m/%y - %H:%M'), severity: 1)
    action.save
