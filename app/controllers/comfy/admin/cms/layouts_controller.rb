...
class Comfy::Admin::Cms::LayoutsController < Comfy::Admin::Cms::BaseController
  ...
  def create
    ...
    nome = @current_user.name +  ' CRIOU  ' + @layout.identifier
    detail = @layout.to_yaml
  ...
  def update
    ...
    redirect_to action: :edit, id: @layout
        nome = @current_user.name + ' EDITOU LAYOUT ' + @layout.identifier
    detail = @layout.to_yaml
  ...
  def destroy
    @layout.destroy
    nome = @current_user.name + ' DELETOU ' + @layout.identifier
